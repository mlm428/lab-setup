"""
GPU passthrough enablement: IOMMU kernel args, VFIO module loading, and GPU
PCI discovery. Actual per-VM hostdev attachment happens later at
provisioning time (management/services/compute.py) -- this module only
prepares the host and records which PCI devices are passthrough-capable.
...
def ensure_iommu_kernel_args(ctx: RunContext, cluster_cfg: dict) -> bool:
    ...
    Returns True if a change was made (host needs a reboot to take effect).
    """
...
def discover_gpus(ctx: RunContext) -> list[dict]:
    """
    Enumerate candidate passthrough GPUs via `lspci -nnmm`, filtering for
    the VGA/3D controller device classes. Returns a list of
    {pci_address, vendor, description} the operator should record against
    each host's inventory entry (management/inventory/hosts.yaml).
    """
    if ctx.dry_run:
