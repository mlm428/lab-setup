...
class TestNamingHelpers(unittest.TestCase):
    def test_switch_name_scoped_by_mission(self):
        self.assertEqual(switch_name("Mission-Alpha", "control"), "Mission-Alpha-control")

    def test_router_name_scoped_by_mission(self):
        self.assertEqual(router_name("Mission-Alpha"), "Mission-Alpha-router")

    def test_router_port_name_scoped_by_mission_and_network(self):
        self.assertEqual(router_port_name("Mission-Alpha", "control"), "Mission-Alpha-control-rp")

    def test_port_name_scoped_by_mission_vm_network(self):
        self.assertEqual(port_name("Mission-Alpha", "db01", "control"), "Mission-Alpha-db01-control")
...
class TestRenderDomainXml(unittest.TestCase):
    def test_pxe_vm_has_no_disk(self):
        ...
        xml_str = render_domain_xml("M", "worker01", vm, ["52:54:00:10:00:00"], LOCAL_STORAGE)
    ...
    def test_linked_clone_vm_has_disk_ceph(self):
        ...
        xml_str = render_domain_xml("M", "db01", vm, ["52:54:00:10:00:00"], CEPH_STORAGE)
        root = ET.fromstring(xml_str)
    ...
    def test_linked_clone_vm_has_disk_local_qcow2(self):
        vm = VMSpec(name="db01", type=VMType.LINKED_CLONE, cpu=4, memory_mb=8192, interfaces=ifaces("control"), image="golden")
        xml_str = render_domain_xml("M", "db01", vm, ["52:54:00:10:00:00"], LOCAL_STORAGE)
        root = ET.fromstring(xml_str)
        disks = root.findall("./devices/disk")
        self.assertEqual(len(disks), 1)
    ...
    def test_nic_count_and_macs_match_interfaces(self):
        ...
        xml_str = render_domain_xml("M", "vm1", vm, macs, LOCAL_STORAGE)
        root = ET.fromstring(xml_str)
    ...
    def test_wrong_mac_count_raises(self):
        ...
        with self.assertRaises(ValueError):
            render_domain_xml("M", "vm1", vm, ["aa:bb:cc:00:00:00"], LOCAL_STORAGE)

    def test_no_gpu_vm_has_no_hostdev(self):
        vm = VMSpec(name="vm1", type=VMType.PXE, cpu=1, memory_mb=1024, interfaces=ifaces("control"))
        xml_str = render_domain_xml("M", "vm1", vm, ["aa:bb:cc:00:00:00"], LOCAL_STORAGE)
        root = ET.fromstring(xml_str)
    ...
    def test_gpu_vm_gets_mdev_hostdev(self):
        ...
        xml_str = render_domain_xml("M", "render01", vm, ["aa:bb:cc:00:00:00"], LOCAL_STORAGE, gpu_mdev_uuid="uuid-1234")
        root = ET.fromstring(xml_str)
    ...
    def test_gpu_vm_without_mdev_uuid_raises(self):
        vm = VMSpec(name="render01", type=VMType.PXE, cpu=8, memory_mb=32768, interfaces=ifaces("control"), gpu_profile="H100-MIG-3g.40gb")
        with self.assertRaises(ValueError):
            render_domain_xml("M", "render01", vm, ["aa:bb:cc:00:00:00"], LOCAL_STORAGE, gpu_mdev_uuid=None)

    def test_non_gpu_vm_with_mdev_uuid_raises(self):
        vm = VMSpec(name="vm1", type=VMType.PXE, cpu=1, memory_mb=1024, interfaces=ifaces("control"))
        with self.assertRaises(ValueError):
            render_domain_xml("M", "vm1", vm, ["aa:bb:cc:00:00:00"], LOCAL_STORAGE, gpu_mdev_uuid="uuid-1234")
    ...
    def test_interface_uses_ovn_virtualport_with_scoped_port_name(self):
        ...
        xml_str = render_domain_xml("Mission-Alpha", "db01", vm, ["aa:bb:cc:00:00:00"], LOCAL_STORAGE)
        root = ET.fromstring(xml_str)
        interface_id = root.find("./devices/interface/virtualport/parameters").get("interfaceid")
        self.assertEqual(interface_id, "Mission-Alpha-db01-control")
    ...
    def test_well_formed_xml_output(self):
        ...
        xml_str = render_domain_xml("M", "vm1", vm, ["aa:bb:cc:00:00:00"], CEPH_STORAGE)
