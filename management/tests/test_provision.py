...
class TestComputeService(unittest.TestCase):
    def test_define_and_start_vm_renders_and_calls_libvirt_client(self):
        ...
        with mock.patch.object(compute, "libvirt_client") as m_client:
            ...
            compute.define_and_start_vm(mission, "vm1", vm, ["aa:bb:cc:00:00:00"], "h1.cluster.local", storage, gpu_mdev_uuid=None)

        m_client.connect.assert_called_once_with("h1.cluster.local")
...
class TestNetworkingService(unittest.TestCase):
    def test_provision_networks_creates_switch_per_network_and_router(self):
        ...
        with mock.patch.object(networking, "ovn_client") as m_ovn:
            switches = networking.provision_networks(mock.MagicMock(), mission)

        self.assertEqual(m_ovn.ensure_logical_switch.call_count, 2)
        m_ovn.ensure_logical_router.assert_called_once_with(mock.ANY, "M-router")
        self.assertEqual(switches, {"control": "M-control", "storage": "M-storage"})
    ...
    def test_add_vm_ports_uses_resolved_macs_in_interface_order(self):
        ...
        with mock.patch.object(networking, "ovn_client") as m_ovn:
            networking.add_vm_ports(mock.MagicMock(), mission, resolved_macs)

        calls = m_ovn.add_vm_port.call_args_list
        self.assertEqual(len(calls), 2)
        self.assertEqual(calls[0].args[1:], ("M-control", "M-vm1-control", "aa:00"))
        self.assertEqual(calls[1].args[1:], ("M-storage", "M-vm1-storage", "aa:01"))
    ...
    def test_teardown_removes_ports_switches_and_router(self):
        ...
        with mock.patch.object(networking, "ovn_client") as m_ovn:
            networking.teardown_mission_networking(mock.MagicMock(), mission)

        m_ovn.remove_vm_port.assert_called_once_with(mock.ANY, "M-vm1-control")
        m_ovn.remove_logical_switch.assert_called_once_with(mock.ANY, "M-control")
        m_ovn.remove_logical_router.assert_called_once_with(mock.ANY, "M-router")
